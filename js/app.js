(() => {
    ...
    da.init();
    if (document.querySelector(".account__asside")) {
        const formItem = document.querySelector(".account__info");
        document.addEventListener("click", (function(e) {
            if (e.target.closest(".link_close")) formItem.classList.add("_close");
        }));
    }
    const menuBodys = document.querySelector(".account__asside");
    const menuBodysIcon = document.querySelector(".user-header");
    document.addEventListener("click", menu);
    function menu(event) {
        if (event.target.closest(".user-header")) {
            menuBodys.classList.toggle("_active");
            menuBodysIcon.classList.toggle("_active");
            document.documentElement.classList.toggle("menu-asside-open");
        }
        if (!event.target.closest(".account__asside, .user-header")) menuBodys.classList.remove("_active");
    }
    if (document.querySelector(".home__banners")) {
        ...
        var x = setInterval((function() {
            ...
            document.getElementById("demo").innerHTML = `<span class="days">${days}</span>` + `<span class="hours">${hours}</span>` + `<span class="minutes">${minutes}</span>` + `<span class="seconds">${seconds}</span>`;
            if (distance < 0) {
                clearInterval(x);
